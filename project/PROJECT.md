...
| **analytics** | Периодические снапшоты метрик доски + live-агрегации (by_status, progress, workload) |
...
```
GET /api/v1/boards/{board_id}/analytics/overview
  200: { by_status: [{ column_id, column_name, count }], total: int }
  # live-агрегация из таблицы tasks; не кешируется

GET /api/v1/boards/{board_id}/analytics/progress?range=week|month
  200: { range: "week"|"month", done_pct: float,
          trend: [{ date: iso8601, done: int, total: int }] }
  # done_pct = count(tasks where column.name='Done') / total * 100
  # trend — дни диапазона; использует BoardMetricSnapshot если есть, иначе live

GET /api/v1/boards/{board_id}/analytics/workload
  200: { by_assignee: [{ assignee_id: uuid|null, name: str|null,
                          count: int, done: int }] }
  # live-агрегация; null assignee = "Unassigned"

BoardMetricSnapshot (периодический снапшот, пишется snapshot.py → APScheduler/background task):
  { id, board_id, captured_at, total_tasks, done_tasks, by_status: jsonb }
  # snapshot writer запускается раз в час (APScheduler); фронт использует как fallback
```
