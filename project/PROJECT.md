...
- Fuzzy-дедупликация при создании: 409 + список кандидатов если похожая задача ≥60% существует в workspace; `force=true` пропускает проверку
...
POST /api/v1/tasks
  body: { title, description, column_id, priority, tags, assignee_id?, deadline?, force?: bool }
        force=true пропускает проверку похожих задач (повторный запрос после диалога)
  200:  { task }
  409:  { detail: "similar_tasks_found",
          candidates: [{ id, title, column_name, similarity: float 0..1 }] }
        до 5 кандидатов, отсортированных по убыванию similarity, порог ≥ 0.6
        алгоритм: difflib.SequenceMatcher workspace-wide, LIMIT 500 задач
...
### DuplicateModal — fuzzy-дедупликация при создании задачи (FEAT-0006)

`ApiError.body?: unknown` — поле в `ApiError` хранит распарсенный JSON-ответ сервера. При 409 `body.candidates` содержит список похожих задач.

Флоу в `AddTaskForm`:
1. `handleSubmit()` — POST без `force`; при 409 парсит `(e as ApiError).body?.candidates`
2. Показывает `DuplicateModal` с кандидатами
3. Пользователь выбирает кандидата → `onCancel()` (форма размонтируется) + `onOpenTask(id)`
4. «Создать новую» → `await handleSubmit(true)` — POST с `force: true`
5. «Отмена» → закрыть модал, форма остаётся с сохранённым текстом

`DuplicateModal`:
- Exact match (similarity ≥ 1.0): `border-left: 2px solid #FCD34D`, заголовок `#FCD34D`
- Hover: меняет только `borderTopColor`/`Right`/`Bottom` — `borderLeft` у exact match не трогается
- Escape-клавиша через `document.addEventListener` в `useEffect`; backdrop-клик вызывает `onCancel`
- `prop drilling onOpenTask`: `BoardPageClient → KanbanBoard → Column → AddTaskForm`

`handleOpenTask(taskId)` в `BoardPageClient`:
- Ищет задачу в текущем board state через `flatMap`
- Если найдена — открывает TaskModal
- Если нет (race condition, задача удалена) — показывает toast "Задача уже удалена"
