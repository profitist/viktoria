...
export default function WorkspaceSettingsPanel({ workspaceId }: WorkspaceSettingsProps) {
  ...
  async function handleToggle() {
    ...
    try {
      ...
      setSettings(prev => (prev ? { ...prev, automation_enabled: !newValue } : prev));
      showToast("Не удалось обновить настройки. Попробуйте ещё раз.");
    } finally {
      setIsPending(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
        ...
        <Toggle enabled={enabled} disabled={pending} onToggle={onToggle} />
      </div>
    </div>
  );
}
...
function Toggle({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onToggle}
      title={enabled ? "Отключить автоматизацию" : "Включить автоматизацию"}
      style={{
        position: "relative",
        width: "42px",
        height: "24px",
